...
load(strcat(starName, '/ironLines', starName))
...
load(strcat(starName, '/headerExtras', starName))
...
save(strcat(starName, '/times', starName), 'obsTimes', 'obsNights', 'uniqueNights');
...
%And to drifts:
drift = drift(noClouds);
drift = drift(sdoFilter);
drift = drift(numsFilter);
...
save(strcat(starName, '/sdoComp.mat'), 'filling', 'plague','spot', 'vs', 'vsLabels', 'bFlux');
...
load(strcat(starName, '/ironBlazes.mat'))
load(strcat (starName, '/ironWaves.mat'))
...
for k = 1:nObs
    ...
    waveTs = obsTimes(k) - waveTimes;
    waveI(k) = find(waveTs > 0, 1, 'last');
end

%Find corresponding vals
blazes = allBlazes(blazeI, :, :);
waves = allWaves(waveI, :, :);
%Now correcting for drift (m/s), and jpl (km/s)
drift(isnan(drift)) = 0;
rvs = RV_JPL(rvI) + drift/1000;
...
save(strcat(starName, '/propError', starName), 'propError', 'blazeError', '-v7.3')


%JPL correction
c = 299792.458; %speed of light in km/sec
wavelengths = waves .* (1 - rvs / c);
rvTimes = jplTimes(rvI);
save jplRVInfo rvTimes rvs

save(strcat(starName, '/processed', starName), 'normOrders', 'ironA', 'wavelengths', '-v7.3')
