...
for i = 1: numObs
    ...
    if strcmp(val,'SCIENCE') && strcmp(tar, 'Sun')
      ...
      try
      drift(i) = fits.readKeyDbl(f, 'HIERARCH TNG DRS DRIFT SPE RV ');
      catch
          warning('No drift val')
          drift(i) = nan;
      end
      fptr = fitsread(fName);
      for j = 1: numlines
...
save(strcat(starName, '/ironLines', starName), 'allLines', 'julian', 'exptime', 'drift', '-v7.3')
save(strcat(starName,'/headerExtras', starName), 'airmass', 'obsTimes', 'berv', '-v7.3')
